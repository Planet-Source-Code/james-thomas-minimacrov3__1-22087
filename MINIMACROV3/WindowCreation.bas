...
Public Declare Function FindWindowEx Lib "user32" Alias "FindWindowExA" (ByVal hWnd As Long, ByVal hWndChild As Long, ByVal lpszClassName As Any, ByVal lpszWindow As Any) As Long
...
Public old_proc As Long
Public new_proc As Long
...
Private Function MyRegisterClass() As Boolean
    ' La structure WNDCLASS
    Dim wndcls As WNDCLASS
    wndcls.style = CS_BYTEALIGNCLIENT 'CS_HREDRAW + CS_VREDRAW
    ...
    wndcls.lpszClassName = "WindClassthe"
    ' Enregistre la classe
    MyRegisterClass = (RegisterClass(wndcls) <> 0)
End Function
'---- Dé-enregistrement d'une classe


Public Sub MyUnregisterClass()
    UnregisterClass "WindClassthe", App.hInstance
End Sub
...
Private Function MyCreateWindow() As Boolean
    ...
    hWnd = CreateWindowEx(0, "WindClassthe", "The Window", WS_OVERLAPPED, 0, 0, 400, 300, hTaskbar, 0, App.hInstance, ByVal 0&)
    hwnd2 = CreateWindowEx(0, "BUTTON", "A Button", WS_CHILD, 50, 55, 100, 25, hTaskbar, 0, App.hInstance, ByVal 0&)
    'hwnd3 = CreateWindowEx(0, "edit", "Ma zone de texte", WS_CHILD, 50, 25, 100, 25, hWnd, 0, App.hInstance, ByVal 0&)
    lngResult = GetWindowLong(hWnd, GWL_STYLE)
    SetWindowLong hWnd, GWL_STYLE, lngResult And Not &HC00000
...
Private Function MonBoutonProc(ByVal hWnd As Long, ByVal message As Long, ByVal wParam As Long, ByVal lParam As Long) As Long
    Dim x As Integer
    If (message = 533) Then
        x = MsgBox("You clicked the Button", vbOKOnly)
    End If
   
    MonBoutonProc = CallWindowProc(old_proc, hWnd, message, wParam, lParam)
End Function
